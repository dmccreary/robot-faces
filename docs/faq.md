...
The course is deliberately narrow: it focuses entirely on the display, the drawing code, and the design of expression, rather than robot chassis assembly, motors, or navigation. See the [Course Description](course-description.md) for the full list of topics covered, and [About This Book](about.md) for the story behind why it was written.
...
The course is also a reasonable fit for self-directed learners outside a formal club setting, since it's designed for both self-paced study and classroom use. See the [Course Description](course-description.md) for the complete target-audience and prerequisites statement.
...
Programming fundamentals get two full chapters of their own — "MicroPython Fundamentals I" and "MicroPython Fundamentals II" — before the book moves on to drawing shapes on a screen, so you won't be expected to write display code until you understand the language it's written in. See the [Course Description](course-description.md) for the complete prerequisites statement and the [Chapters overview](chapters/index.md) for where those lessons fall in the sequence.
...
The electronics knowledge you do need — how to identify a pin, how SPI wiring works, what a breadboard's power rails are for — is taught starting in the very first chapter, [Hardware & Electronics Foundations](chapters/01-hardware-electronics-foundations/index.md). That chapter walks through wiring a display to a Raspberry Pi Pico using five SPI pins (SCL, SDA, DC, RES, CS) before any code is written, so by the time you reach programming, the physical connections are already familiar.
...
The minimal robot face kit has just four parts, all listed on the [Getting Started](getting-started.md) page:
...
Prices vary somewhat with tariffs, shipping method, and how many units you buy at once — ordering from AliExpress or eBay in bulk lots is typically cheaper per unit but takes 3-5 weeks to arrive, while Amazon or MicroCenter cost a bit more but ship in days. The [Parts List](parts-list.md) has current sourcing notes, including tips for outfitting a whole classroom of 10-30 students at once. See [About This Book](about.md) for more on why keeping the cost this low was a deliberate design goal.
...
If you want the full breakdown of what each chapter covers, see the [Chapters overview](chapters/index.md). For a page-by-page tour of every section of the site, see the [homepage](index.md), which lists Getting Started, Chapters, Lessons, Stories, the Learning Graph, the Glossary, and the Parts List.
...
A single class period is usually enough to get through the hardware setup in [Getting Started](getting-started.md) and see your first face appear on the screen — that part is intentionally fast so students get a payoff quickly. The chapters that follow go deeper and take longer: MicroPython fundamentals, drawing primitives, and facial anatomy each take real practice, and the animation and interactive-controls chapters near the end assume you're comfortable with everything before them.
...
Setup has four steps, covered in detail on the [Getting Started](getting-started.md) page:
...
You will need a USB cable to connect the Pico to your computer, which is not always included with the microcontroller, so it's worth checking the [Parts List](parts-list.md) or your own drawer for a compatible one before class starts.
...
See the [Course Description](course-description.md) for the complete list of learning outcomes, organized from basic recall up through this capstone-level creative work.
...
Teachers and club mentors can also embed any MicroSim in an external classroom page or LMS using a plain `<iframe>`. See [About This Book](about.md) for more on how MicroSims fit into the book's overall design.

### Where should I start if I want to begin building right away?

If you want to get a face on a screen as fast as possible rather than reading background material first, start with the [Getting Started](getting-started.md) page. It walks through ordering the four-part minimal kit, installing the Thonny editor, and copying a ready-made program called simple-face.py onto your Raspberry Pi Pico — most students see their first face appear on the display within a single class period.

After that first face is working, move on to [Chapter 1: Hardware & Electronics Foundations](chapters/01-hardware-electronics-foundations/index.md), which explains the wiring and components you just used in more depth before the course moves into MicroPython programming and drawing code in the chapters that follow.

If you'd rather explore before committing to a linear path — for example, to preview what a finished capstone project looks like, or to check what a specific chapter assumes you already know — the [homepage](index.md) lists every section of the book, including Chapters, Lessons, Stories, the Learning Graph, and the Glossary, so you can browse before you dive in. Either path is reasonable: the course is built to reward jumping straight into hardware, but it also rewards a slower survey first.
...
More on wiring both displays appears in [Hardware & Electronics Foundations](chapters/01-hardware-electronics-foundations/index.md).
...
Both displays are introduced together in [Hardware & Electronics Foundations](chapters/01-hardware-electronics-foundations/index.md), and the overall course scope is summarized in the [course description](course-description.md).
...
This history is covered in depth in [A History of Screen-Based Robot Faces](chapters/02-history-of-robot-faces/index.md).
...
Social robots and the companion robot category are discussed in [A History of Screen-Based Robot Faces](chapters/02-history-of-robot-faces/index.md).
...
Variables and constants are introduced in [MicroPython Fundamentals I: Syntax, Data & Loops](chapters/03-micropython-fundamentals-1/index.md).
...
Functions are introduced in [MicroPython Fundamentals II: Functions & the FrameBuf Module](chapters/04-micropython-fundamentals-2/index.md).
...
The frame buffer and show method are explained in [Display & Coordinate Systems](chapters/05-display-coordinate-systems/index.md).
...
Pixels and resolution are covered in [Display & Coordinate Systems](chapters/05-display-coordinate-systems/index.md).
...
This coordinate convention is explained in detail in [Display & Coordinate Systems](chapters/05-display-coordinate-systems/index.md).
...
Drawing color values are introduced in [Basic Drawing Primitives](chapters/06-basic-drawing-primitives/index.md).
...
The rectangle and filled rectangle methods are covered in [Basic Drawing Primitives](chapters/06-basic-drawing-primitives/index.md).
...
The blit method and sprites are explained in [Basic Drawing Primitives](chapters/06-basic-drawing-primitives/index.md).
...
The ellipse method and its parameters are covered in [Ellipse & Polygon Drawing](chapters/07-ellipse-polygon-drawing/index.md).
...
Both methods are compared in detail in [Ellipse & Polygon Drawing](chapters/07-ellipse-polygon-drawing/index.md).
...
The full version history is traced in [A History of MicroPython's FrameBuf Drawing Support](chapters/08-framebuf-version-history/index.md).
...
Face outline and layout planning are covered in [Facial Anatomy & Layout Design](chapters/09-facial-anatomy-layout-design/index.md).
...
Eyebrow shape and its role in expression are discussed in [Facial Anatomy & Layout Design](chapters/09-facial-anatomy-layout-design/index.md).
...
Mouth shape and curvature are covered in [Facial Anatomy & Layout Design](chapters/09-facial-anatomy-layout-design/index.md).
...
Eye and pupil placement are discussed in [Facial Anatomy & Layout Design](chapters/09-facial-anatomy-layout-design/index.md).
...
Parameterized face design is introduced in [Facial Anatomy & Layout Design](chapters/09-facial-anatomy-layout-design/index.md).
...
The draw face function is covered in detail in [Facial Anatomy & Layout Design](chapters/09-facial-anatomy-layout-design/index.md).
...
The neutral expression and the full core expression set are covered in [Emotion Theory & the Core Expression Set](chapters/10-emotion-theory-core-expressions/index.md).
...
Expression intensity is discussed in [Expression Design, Readability & Human-Robot Interaction](chapters/11-expression-design-readability-hri/index.md).

### How would I structure an animation loop to make a robot face blink?
...
Building an animation loop is covered step by step in [Animating Expressions: Timing & Motion](chapters/12-animating-expressions/index.md).
...
Mode state machines are introduced in [Interactive Controls: Inputs & Concurrency](chapters/13-interactive-controls-inputs/index.md).
...
RGB565 and the color565() function are explained in [Porting Faces to a Color Display](chapters/15-porting-faces-color-display/index.md).
...
Computational thinking is the focus of [Computational Thinking & Capstone Design](chapters/16-computational-thinking-capstone/index.md).
...
Expression planning for the capstone is covered in [Computational Thinking & Capstone Design](chapters/16-computational-thinking-capstone/index.md).
...
Variables, constants, and these core data types are introduced in [MicroPython Fundamentals I](chapters/03-micropython-fundamentals-1/index.md).
...
See [MicroPython Fundamentals I](chapters/03-micropython-fundamentals-1/index.md) for a full walkthrough of getting connected.
...
Both concepts are introduced together in [MicroPython Fundamentals II](chapters/04-micropython-fundamentals-2/index.md).
...
See [Display & Coordinate Systems](chapters/05-display-coordinate-systems/index.md) for the full sequence used with both displays.
...
[Hardware & Electronics Foundations](chapters/01-hardware-electronics-foundations/index.md) walks through pricing out a complete kit.
...
See [Basic Drawing Primitives](chapters/06-basic-drawing-primitives/index.md) for more on layering drawing calls correctly.
...
See [A History of MicroPython's FrameBuf Drawing Support](chapters/08-framebuf-version-history/index.md) for the complete timeline.
...
See [Facial Anatomy & Layout Design](chapters/09-facial-anatomy-layout-design/index.md) for how both parameters fit into a complete eye design.
...
See [Emotion Theory & the Core Expression Set](chapters/10-emotion-theory-core-expressions/index.md) for the full expression set and the theory behind it.
...
See [A History of Screen-Based Robot Faces](chapters/02-history-of-robot-faces/index.md).
...
See [Animating Expressions: Timing & Motion](chapters/12-animating-expressions/index.md) for how these fit into a non-blocking animation loop.
...
See [Interactive Controls: Inputs & Concurrency](chapters/13-interactive-controls-inputs/index.md) and [Building an Expression Menu & Live Controls](chapters/14-expression-menu-live-controls/index.md).
...
See [Porting Faces to a Color Display](chapters/15-porting-faces-color-display/index.md).
...
See [Computational Thinking & Capstone Design](chapters/16-computational-thinking-capstone/index.md).
...
See [A History of Screen-Based Robot Faces](chapters/02-history-of-robot-faces/index.md) for real case studies.
...
See [MicroPython Fundamentals I](chapters/03-micropython-fundamentals-1/index.md) and [MicroPython Fundamentals II](chapters/04-micropython-fundamentals-2/index.md).
...
See [Display & Coordinate Systems](chapters/05-display-coordinate-systems/index.md) for a full comparison of the two displays.
...
See [Hardware & Electronics Foundations](chapters/01-hardware-electronics-foundations/index.md) for the full wiring diagram.
...
See [Basic Drawing Primitives](chapters/06-basic-drawing-primitives/index.md) for the complete set of drawing methods.
...
See [Basic Drawing Primitives](chapters/06-basic-drawing-primitives/index.md) for how these extend basic drawing into sprites and motion.
...
See [Ellipse & Polygon Drawing](chapters/07-ellipse-polygon-drawing/index.md) for worked examples of both.
...
See [Emotion Theory & the Core Expression Set](chapters/10-emotion-theory-core-expressions/index.md) for how these ideas ground the course's designs.
...
See [Emotion Theory & the Core Expression Set](chapters/10-emotion-theory-core-expressions/index.md) for the full recipe of all thirteen expressions.
...
See [Emotion Theory & the Core Expression Set](chapters/10-emotion-theory-core-expressions/index.md) for the drawing recipe of each.
...
Whenever a variable seems to silently "reset" or "not update" even though your code looks correct, check whether you're assigning to it inside a function without declaring it `global` first. See [Global Versus Local Scope](chapters/04-micropython-fundamentals-2/index.md) for a full walkthrough of this bug.
...
**Cooperative multitasking** on the RP2040 works because MicroPython does not force safety on shared data — it only works if the program's own discipline keeps both cores from colliding. A **global state variable** like `current_mode`, described in [Interactive Controls: Inputs & Concurrency](chapters/13-interactive-controls-inputs/index.md), is exactly the kind of value that can go wrong under **dual-core processing**: if core 1's input-polling task advances `current_mode` from `GAZE_MODE` to `ADJUST_MODE` at the same instant core 0 is reading that variable to decide what to draw, core 0 could act on a value that is only partway updated, or read the old value and act on stale information for one extra frame.
...
[Computational Thinking & Capstone Design](chapters/16-computational-thinking-capstone/index.md) names this tension directly: **expressiveness versus complexity** is the trade-off between adding more visual richness and keeping a project simple enough to finish, debug, and demonstrate with confidence. Judging where a specific design falls on that spectrum is an evaluate-level skill, and it comes down to testing against concrete criteria rather than a gut feeling.
