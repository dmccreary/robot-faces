...
#
# This lab turns the face into its own instrument. A heads-up display in
# the corner reports four numbers that tell you what the program is really
# doing, and the same numbers go to the Thonny shell once a second so you
# have a record you can scroll back through.
#
...
def draw(a_value, b_value):
    """Draw the face plus the instrument panel. Everything the program
    knows about itself is on the glass, where you can watch it change."""
    face.clear()
...
#
# 2. With SLOW_MODE on, tap button A as fast as you can ten times. Compare
#    the "hit" counter to ten. Every missing press was swallowed by a
#    sleep().
#
# 3. Lower SLOW_DELAY_MS until presses stop getting lost. The number you
#    land on is roughly how long a human finger stays on a button -- you
#    just measured a person with a microcontroller.
#
# 4. Comment out the draw() call for ten seconds and watch fps jump. Now
#    you know how much of your program's time is spent talking to the
#    display, which is exactly the question lab 29 answers.
#
# 5. Add your own number to the panel -- how long the last draw() took, in
#    milliseconds. Wrap it in ticks_ms() readings the same way fps is
#    measured here.
