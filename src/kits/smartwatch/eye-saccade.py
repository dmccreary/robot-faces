...
# you make it, and why this program looks alive while doing less work.
#
# THE COST, WHICH IS THE SURPRISING PART. Lab 11 redraws the eyes on
# every one of the 60 frames it takes to cross the face, and then turns
# around and does it again, forever. This program crosses in at most 12
# frames and then draws NOTHING AT ALL for a third of a second.
#
# So the expensive redraw stops mattering. Erasing two 92 x 56 boxes and
# rebuilding both eyes is a lot of pixels -- but doing it a dozen times
# and then resting is far cheaper than doing it sixty times and starting
# over. RARE WORK IS ALLOWED TO BE EXPENSIVE. That is worth remembering
# every time you are tempted to optimize something that hardly ever runs.
#
...
def draw_static_parts():
    """Everything that never moves. Drawn once, then left alone."""
    display.fill(OFF)
    draw_mouth()


def draw_eyes(offset):
    """Erase the two eye boxes and rebuild them -- exactly what lab 11
    does. Unchanged on purpose: this lab is about WHEN you draw, not how.

    Both eyes always look the same direction, which is what makes a face
    read as looking AT something rather than in two directions at once."""
...
#
# 3. Set ERASE_COLOR = config.RED. Now you can see exactly how much
#    repainting one saccade costs -- and, more importantly, how much of
#    the time the screen is completely still. That stillness is the
#    saving.
#
# 4. Set SACCADE_STEP to 1. The jump becomes a slow glide, the printed
#    microseconds jump by five times, and the face stops looking alive. Slower,
#    smoother and more expensive, all at once -- a rare chance to make
#    something worse in three ways with one number.
#
